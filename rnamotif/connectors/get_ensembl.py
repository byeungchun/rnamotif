    ...
    for gene in lst_gene:
        ...
        try:
            while num_retry < MAX_RETRY:
                # Request API service
                ret = requests.get(req_url, headers=HEADER,verify=is_ssl_verification)
                if ret.status_code == 200:
                    ...
                    logger.info(f"Ensembl API server is not working for {req_url}.\nWait 5 seconds and retry. # of retries: {num_retry}/{MAX_RETRY}")
                    time.sleep(5)
                    num_retry += 1
                    continue
                    ...
                    logger.error(f'Unknown server status code({ret.status_code})')
                
                
                res[gene][seq_type] = {'retsts': flag_req_success, 'retval': ret.text}
                break
        except Exception as ex:
            RuntimeError(f'Ensembl API request error.\n{ex}')
        logger.info(f'FINISH - {req_url}')
    return res
